...
- 已获取用于更新固件的文件。STCP920的固件包括NPU ctrl firmware和MCU firmware。
...
已规范稳定版固件更新文件的名称，在文件名称中添加字段区分产品，格式为(product_type)-(firmware_type)-(firmware_version).sdux。示例如下：

- [NPU ctrl firmware示例](http://pm.streamcomputing.com/browse/JZNPU-2656)：STCP908-Flash-EVB2B-ddr3733M-soc900M-v1.2.2-4399e55.sdux
- [MCU firmware示例](http://pm.streamcomputing.com/browse/JZNPU-2657)：STCP920_MCU_V1.0.6_163456.sdux

使用stc-smi命令即可更新固件版本，可升级版本和回滚版本，同时请注意以下方面：

- 待更新固件：STCP920的固件包括NPU ctrl firmware和MCU firmware，支持分别更新升级和回滚，通过命令选项指定即可。指定NPU ctrl firmware的形式为`-u {file_name}.sdux`，指定MCU firmware的形式为`--mcu-upgrade {file_name}.sdux`。
...
| 小于V1.3.4                    | 大于等于V1.3.4                | 更新前小于等于V1.7.2更新后大于等于V1.8.0 | *更新流程2*  |
| 大于等于V1.3.4                | 小于V1.3.4                    | 更新前小于等于V1.7.2更新后无要求         | *更新流程3*  |
| 小于V1.3.4                    | 小于V1.3.4                    | 更新前小于等于V1.7.2更新后无要求         | *更新流程3*  |
...
3. 执行`stc-smi --mcu-upgrade {file_name}.sdux -i {device_id}`命令，为指定NPU设备更新MCU firmware。
