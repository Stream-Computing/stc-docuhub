...
firmware预装在各个硬件设备上用于直接控制设备，并供驱动程序加载以和更上层的软件进行交互。其中预装了以下firmware：

* NPU ctrl firmware：负责初始化NPU的SoC系统。

* MCU firmware：配合主机管理PCIe卡的电源供应。

为满足功能需求或解决问题，我们会发布新版本的firmware，为已有设备升级firmware的具体操作，请参见[固件更新指南](https://docs.streamcomputing.com/_/sharing/vSxLMI20nalGphdpXdEVoDg6JkUcfEkT?next=/zh/latest/)。

## NPU ctrl firmware功能变化

* V10.3.8：统一STCP980L型号。

* V10.3.7：

  * 新增STCP950对应的PCIe Device ID和Sub-System ID。
...
* V10.0.14：新增STCP980L/P的产品部件号Part Number、PCIe Device ID和Sub-System ID。
