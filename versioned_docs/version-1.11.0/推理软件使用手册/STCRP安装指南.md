...
* 吹风方向
  ...
  ![](\\doc_img\\version-1.11.0\\STCRP安装指南-s9f60MWIqjNL.png)

* 吸风方向

  ![](\\doc_img\\version-1.11.0\\STCRP安装指南-EW4J1NyWLOpD.png)
...
7. 验证是否已成功安装推理卡。执行以下命令检查推理卡的安装状态，回显设备信息则说明操作系统已发现设备。下方为发现NPU设备的示例：
...
* 解决方法：在BIOS面板中启用`Above 4G Decoding`，如下所示：
  ...
  ![](\\doc_img\\version-1.11.0\\STCRP安装指南-img_v3_02ei_4f107f7d-e4ee-41a9-8419-40d3cd03b9dg.jpg)
...
<th><strong>GCC版本</strong></th>
<th><strong>dkms版本</strong></th>
</tr>
</thead>
<tbody>
<tr>
...
<tr>
<td>Ubuntu 25.04</td>
<td>6.14.0-15-generic</td>
<td>2.41</td>
<td>14.2.0</td>
<td>3.0.11</td>
</tr>
<tr>
<td>银河麒麟高级服务器操作系统V10（以下简称麒麟V10）</td>
<td>4.19.90<br /></td>
<td>2.31<br /></td>
<td><ul> <li>7.3.0</li> <li>9.0.0（ MLTC要求）</li> </ul></td>
<td>2.6.1-6<br /></td>
</tr>
</tbody>
</table>
...
<thead>
<tr>
<th><strong>配套软件</strong></th>
<th><strong>安装包名称格式</strong></th>
</tr>
</thead>
<tbody>
<tr>
<td>HPE<br /></td>
<td><ul> <li>deb包名称格式：<code>hpe-repo-&#123;OS&#125;-&#123;main_version&#125;-local_&#123;sub_version&#125;_&#123;arch_version&#125;.deb</code></li> <li>HPE整包中包括了驱动以及stc-smi、stc-gdb、stc-prof、stc-vprof等监控调试工具，支持通过命令控制安装的模块分类。</li> </ul></td>
</tr>
<tr>
<td>HPE Python</td>
<td><code>hpe_python-&#123;mltc_version&#125;-cp&#123;python_version&#125;-&#123;abi&#125;-linux_&#123;arch_version&#125;.whl</code></td>
</tr>
<tr>
<td>MLTC</td>
<td><code>mltc-&#123;mltc_version&#125;-cp&#123;python_version&#125;-&#123;abi&#125;-linux_&#123;arch_version&#125;.whl</code></td>
</tr>
<tr>
<td>SNQ</td>
<td><code>stcnq-&#123;SNQ_version&#125;-py3-none-any.whl</code></td>
</tr>
<tr>
<td>SNC</td>
<td><code>stcnc-&#123;SNC_version&#125;-py3-none-any.whl</code></td>
</tr>
<tr>
<td>STC_LLM</td>
<td><code>stc_llm-&#123;stc_llm_version&#125;-py&#123;python_version&#125;-&#123;abi&#125;-linux_x86_64.whl</code></td>
</tr>
<tr>
<td>STC_LLM_DNN</td>
<td><code>stc_llm_dnn-&#123;stc_llm_dnn_version&#125;-py&#123;python_version&#125;-&#123;abi&#125;-linux_x86_64.whl</code></td>
</tr>
<tr>
<td>STC_IE</td>
<td><code>stc_ie-&#123;stc_ie_version&#125;-py&#123;python_version&#125;-&#123;abi&#125;-linux_x86_64.whl</code></td>
</tr>
<tr>
<td>其他安装用附加文件<br /></td>
<td><ul> <li>Dockerfile：Docker方式安装时构建Docker镜像的示例。</li> <li>requirements：模型编译依赖，按需从requirements.txt、requirements_venv2.txt等中选择。</li> </ul></td>
</tr>
</tbody>
</table>
...
| **配套软件**      | **安装包名称示例**                                      |
| ------------- | ------------------------------------------------ |
| HPE           | hpe-repo-ubuntu2204-1-9-local_1.9.5_amd64.deb  |
| HPE Python    | hpe_python-1.4.1-cp310-cp310-linux_x86_64.whl |
| MLTC          | mltc-1.5.0-cp310-cp310-linux_x86_64.whl        |
| SNQ           | stcnq-1.0.0-py3-none-any.whl                     |
| SNC           | stcnc-1.0.1-py3-none-any.whl                     |
| STC_LLM      | stc_llm-1.3.1-py3-none-any.whl                  |
| STC_LLM_DNN | stc_llm_dnn-1.3.0-py3-none-any.whl             |
| STC_IE       | stc_ie-1.5.0-py3-none-any.whl                   |
...
* Ubuntu 22.04
  ...
  ```bash
  $ sudo dpkg -i hpe-repo-ubuntu2204-1-9-local_1.9.5_amd64.deb
...
* Ubuntu 25.04
  ...
  ```bash
  $ sudo dpkg -i hpe-repo-ubuntu2504-1-9-local_1.9.5_amd64.deb
...
* 麒麟V10
  ...
  ```bash
  $ sudo yum install hpe-repo-kylinV10-1-9-local_1.9.5_x86_64.rpm
...
1. 构建Docker镜像。示例流程如下：
   ...
   2. 在主机中操作构建Docker镜像。构建命令中指定必要的信息，例如Dockerfile路径、镜像名称、安装包获取路径等。以Ubuntu 22.04为例，命令及主要参数说明如下：
      ...
      ```bash
      $ DOCKER_BUILDKIT=1 docker build -f Dockerfile -t stcrp1110:ubuntu2204-hpe-1.9.5-mltc-1.5.0-py310 --build-arg URI=http://example.com --target final .
      ```

      * `-f Dockerfile`：指定使用当前目录下的Dockerfile。

      * `-t stcrp1110:ubuntu2204-hpe-1.9.5-mltc-1.5.0-py310`：指定Docker镜像的名称为stcrp1110，标签为ubuntu2204-hpe-v1.9.5-mltc-v1.5.0-py310。
   ...
   3. 查看构建好的Docker镜像。
      ...
      stcrp1110         ubuntu2204-hpe-1.9.5-mltc-1.5.0-py310  cacd5995dcb9   5 minutes ago   10.7GB
      ```

2. 基于Docker镜像启动容器。启动容器时，除Docker的原生参数外，还支持指定容器占用的NPU设备。以为容器分配1个NPU设备的4个Cluster为例：

   ```bash
   $ sudo docker run -it --device /dev/stc0 --device /dev/stc0c0 --device /dev/stc0c1 --device /dev/stc0c2 --device /dev/stc0c3 --device /dev/stc0ctrl stcrp1110:ubuntu2204-hpe-1.9.5-mltc-1.5.0-py310 /bin/bash
   ```
   ...
   * `stcrp1110:ubuntu2204-hpe-1.9.5-mltc-1.5.0-py310`：指定使用的Docker镜像。
...
建议先安装HPE再安装MLTC及其他工具，配套软件之间存在版本对应关系，详细信息请参见[STCRP Release Notes](https://docs.streamcomputing.com/_/sharing/vSxLMI20nalGphdpXdEVoDg6JkUcfEkT?next=/zh/latest/)。
...
> 注意：如果在虚拟机中使用设备，首次启动虚拟机后，建议在虚拟机中重启一次推理卡，否则可能无法运行hello_world。
...
```bash
$ pip3 install hpe_python-1.4.1-cp310-cp310-linux_x86_64.whl
```
...
2. 安装指定版本的torch-mlir。
   ...
   ```bash
   $ pip3 install torch_mlir-20251004.590-cp310-cp310-manylinux2014_x86_64.manylinux_2_17_x86_64.whl
   ```

3. 将MLTC及其他工具的安装包复制到服务器，执行安装命令即可。示例命令如下：

   ```bash
   $ pip3 install mltc-1.5.0-cp310-cp310-linux_x86_64.whl
   ```
...
* STC_LLM依赖HPE Python、STC_LLM_DNN、STC_IE，需一同安装。

* 安装STC_LLM之前，请确保已安装了HPE Python。

* 安装STC_IE之前，请确保已安装了MLTC。

#### 安装步骤

1. 将STC_LLM、STC_LLM_DNN、STC_IE安装包复制到目标服务器。

2. 安装STC_LLM。

   ```bash
   $ pip3 install stc_llm-1.3.1-py3-none-any.whl
   ```
...
3. 安装所需的库。
   ...
   * 安装STC_LLM_DNN。
     ...
     ```bash
     $ pip3 install stc_llm_dnn-1.3.0-py3-none-any.whl
     ```

   * 安装STC_IE。

     ```bash
     $ pip3 install stc_ie-1.5.0-py3-none-any.whl
     ```

#### 验证安装结果

查看STC_LLM、STC_LLM_DNN、STC_IE模块，回显模块及版本即表示已成功安装。
...
1. 查看已安装STC_LLM、STC_LLM_DNN、STC_IE模块。
