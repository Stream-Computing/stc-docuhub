...
希姆计算STCP980L AI推理加速卡基于NeuralScale架构、软硬件协同设计，可以通过标准的PCIe接口便捷地集成到各类服务器中，从而快速扩展云端推理能力，满足计算机视觉、自然语言处理、语音识别、搜索推荐、LLM等主流AI场景中的推理加速需求。
...
STCP980L AI推理卡遵从标准PCIe 4.0协议（PCI Express Card Electromechanical Specification Revision 4.0），需要为整卡提供信号质量满足PCIe 4.0规范（向下兼容PCIe 3.0、PCIe 2.0、PCIe 1.0）的同源时钟。

## 硬件设计

### 整体规格

STCP980L AI推理卡的整体规格如下：

| **规格项** | **规格说明**                                                 |
| ---------- | ------------------------------------------------------------ |
| 标识       | 型号：STCP980L<br/>产地：中国                                |
| 外观       | 尺寸：单宽、3/4长、全高（268.44mm × 111.15mm）<br/>重量：721.2 g |
| 功耗       | 功耗：160 W<br/>电源：一个PCIe 8-Pin电源连接器<br/>散热：被动散热 |
| 性能       | 核心架构：Neural Scale NPC<br/>核心频率：1 GHz<br/>最大频率：1.4 GHz<br/>半精度浮点运算速度（FP16）：179TFLOPS<br/>整数运算速度（INT8）：358TOPS |
| 内存       | 内存类型：LPDDR4X<br/>内存容量：16 GB<br/>内存位宽：256-bit<br/>内存时钟频率：1867 MHz<br/>内存带宽：108 GB/s |
| 接口       | PCIe接口：PCIe 4.0 × 16，支持Lane Reversal。<br/>PCI标识符：<br/>- PCIe Vendor ID：0x23e2<br/>- PCIe Device ID：0x0105<br/>- PCIe Sub-Vendor ID：0x23e2<br/>- PCIe Sub-System ID：0x0005<br/>CTO（PCIe Completion Timeout）功能：<br/>- 支持的range（DEVCAP2）：10 ms - 250 ms<br/>- 是否有特殊设定需求（DEVCTL2）：默认Completion Timeout值为200 ms |

### 尺寸规格

STCP980L AI推理卡的尺寸规格为单宽、3/4长、全高（268.44mm × 111.15mm），示意图如下：

![](\doc_img\version-1.10.0\image-1.png)

### 电源规格

STCP980L AI推理卡提供一个PCIe 8-Pin的电源接口，您可以直接使用服务器内的PCIe 8-Pin电源插头连接到AI推理卡的外接电源连接器上。
...
STCP980L AI推理卡采用被动散热，需要系统风扇提供冷却风量。芯片内部集成有温度监测点来检测温度，同时板卡的入风口和出风口均有温度监测芯片，可以实时监控温度，便于及时、有效的进行散热。
...
STCP980L AI推理卡支持在机箱的前端及尾端插卡，板卡支持双向风流，可选的进风风流方向如下：
...
| 固件版本号                           | 0xF5     | RO           | 4 Byte   | 读取MCU固件版本号，数据类型UINT\_8，取值示例为10.0.13。 |
...
| 产品部件号                           | 0xFA     | RO           | 16 Byte  | 读取版本部件号，数据类型char，取值示例为980L032210830。 |
| 芯片电压                             | 0x50     | RO           | 4 Byte   | 读取芯片电压，数据类型UINT\_32，单位mV。                |
| MAC电压                              | 0x51     | RO           | 4 Byte   | 读取MAC电压，数据类型UINT\_32，单位mV。                 |
...
围绕STCP980L的全栈软硬件架构图如下：

![](\doc_img\version-1.10.0\image-16.png)
