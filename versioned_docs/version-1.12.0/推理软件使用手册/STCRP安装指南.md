...
| **配套软件**      | **安装包名称示例**                                      |
| ------------- | ------------------------------------------------ |
| HPE           | hpe-repo-ubuntu2204-1-9-local_1.9.6_amd64.deb  |
| HPE Python    | hpe_python-1.4.1-cp310-cp310-linux_x86_64.whl |
| MLTC          | mltc-1.6.0-cp310-cp310-linux_x86_64.whl        |
...
| STC_IE       | stc_ie-1.6.0-py3-none-any.whl                   |
...
* Ubuntu 22.04
  ...
  ```bash
  $ sudo dpkg -i hpe-repo-ubuntu2204-1-9-local_1.9.6_amd64.deb
...
* Ubuntu 25.04
  ...
  ```bash
  $ sudo dpkg -i hpe-repo-ubuntu2504-1-9-local_1.9.6_amd64.deb
...
* 麒麟V10
  ...
  ```bash
  $ sudo yum install hpe-repo-kylinV10-1-9-local_1.9.6_x86_64.rpm
...
1. 构建Docker镜像。示例流程如下：
   ...
   2. 在主机中操作构建Docker镜像。构建命令中指定必要的信息，例如Dockerfile路径、镜像名称、安装包获取路径等。以Ubuntu 22.04为例，命令及主要参数说明如下：
      ...
      ```bash
      $ DOCKER_BUILDKIT=1 docker build -f Dockerfile -t stcrp1120:ubuntu2204-hpe-1.9.6-mltc-1.6.0-py310 --build-arg URI=http://example.com --target final .
      ```

      * `-f Dockerfile`：指定使用当前目录下的Dockerfile。

      * `-t stcrp1110:ubuntu2204-hpe-1.9.6-mltc-1.6.0-py310`：指定Docker镜像的名称为stcrp1120，标签为ubuntu2204-hpe-v1.9.6-mltc-v1.6.0-py310。
   ...
   3. 查看构建好的Docker镜像。
      ...
      stcrp1120         ubuntu2204-hpe-1.9.6-mltc-1.6.0-py310  cacd5995dcb9   5 minutes ago   10.7GB
      ```

2. 基于Docker镜像启动容器。启动容器时，除Docker的原生参数外，还支持指定容器占用的NPU设备。以为容器分配1个NPU设备的4个Cluster为例：

   ```bash
   $ sudo docker run -it --device /dev/stc0 --device /dev/stc0c0 --device /dev/stc0c1 --device /dev/stc0c2 --device /dev/stc0c3 --device /dev/stc0ctrl stcrp1120:ubuntu2204-hpe-1.9.6-mltc-1.6.0-py310 /bin/bash
   ```
   ...
   * `stcrp1120:ubuntu2204-hpe-1.9.6-mltc-1.6.0-py310`：指定使用的Docker镜像。
...
3. 将MLTC及其他工具的安装包复制到服务器，执行安装命令即可。示例命令如下：
   ...
   ```bash
   $ pip3 install mltc-1.6.0-cp310-cp310-linux_x86_64.whl
   ```
...
3. 安装所需的库。
   ...
   * 安装STC_IE。
     ...
     ```bash
     $ pip3 install stc_ie-1.6.0-py3-none-any.whl
     ```
