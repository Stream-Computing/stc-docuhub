...
准备好模型推理环境，安装HPE、MLTC、Python 3.10。具体操作，请参见*STCRP安装指南*。
...
调用`optimize`接口将AI框架导出的模型文件，转换为MLTC自定义的图层IR并编译部署。
...
import numpy as np
import torch
from mltc.mltc.frontend.torch_frontend import optimize
...
class TestModule(torch.nn.Module):
    def forward(self, a: torch.Tensor) -> torch.Tensor:
        x = torch.ops.mylib.numpy_sin(a)
        y = torch.ops.aten.sin(a)
        return x, y

if __name__ == "__main__":
    model = TestModule().eval()
    input = torch.rand((3, 4), dtype=torch.float)
    ...
    with torch.no_grad():
        ...
        mltc_out = optimized_model(input)

    optimized_model.release()  
    print(model.__class__.__name__)
    print("mltc output")
    print(mltc_out)
    assert all(torch.allclose(m, t, rtol=1e-2, atol=1e-2) for m, t in zip(mltc_out, torch_out))
```
...
![](\doc_img\version-1.9.0\mltc-image-13.png)
...
指令编码翻译脚本工具（inst\_decoder.py）放在MLTC安装目录下的`tools`下。使用脚本解析报错的指令编码，得到指令编码对应的具体指令。
...
![](\doc_img\version-1.9.0\mltc-image-12.png)
...
2. 从[HuggingFace](https://huggingface.co/fcakyon/yolov5s-v7.0/tree/main)下载pt格式的权重（`yolov5s.pt`），并将权重文件移动到YOLOv5 repo所在的平级目录。
   ...
   ![](\doc_img\version-1.9.0\mltc-image-14.png)
...
6. 验证ONNX模型在CPU上的执行效果。
   ...
   ![](\doc_img\version-1.9.0\mltc-image-11.png)
...
9. 修改部分代码适配YOLOv5的repo。
   ...
   * 修改`models/common.py`，修改点如下：
     ...
     ![](\doc_img\version-1.9.0\mltc-image.png)
   ...
   * 修改`export.py`，修改点如下：
     ...
     ![](\doc_img\version-1.9.0\mltc-image-1.png)
...
![](\doc_img\version-1.9.0\mltc-image-2.png)

![](\doc_img\version-1.9.0\mltc-image-3.png)
...
目前性能分析工具，不支持copy类算子及DMA cycle性能数据统计。具体支持的算子参见*MLTC算子支持说明*。
...
* dump\_data文件夹：主要保存的是dump出来的16进制的数据。

* result文件夹：主要保存的是最终对比分析结果。

  * `skip_perf_dict.csv`：执行过程中被跳过的节点以及详细跳过信息。

  * `op_perf_{coreid}.csv`：每个核上的性能数据。

以`op_perf\_0.csv`性能数据为例，结果文件如下图所示：

![](\doc_img\version-1.9.0\mltc-image-4.png)
...
| name                   | 节点名称。                     |
| mcu\_cycle             | 总的执行周期数。                  |
| vme\_cycle             | vme执行单元的周期数。              |
| mme\_cycle             | mme执行单元的周期数。              |
| vec\_cycle             | rvv执行单元的周期数。              |
| syn\_cycle             | sync执行单元的周期数。             |
| mte\_total\_cycle      | mte总周期数。                  |
| vme\_inst              | vme指令条数。                  |
| mme\_inst              | mme指令条数。                  |
| vec\_inst              | rvv指令条数。                  |
| syn\_inst              | sync指令条数。                 |
| mte\_pld\_inst         | pld指令条数。                  |
| mte\_icmov\_inst       | icmov指令条数。                |
| mte\_mov\_inst         | mov指令条数。                  |
| pal\_vme\_mme\_cycle   | vme和mme指令并行运行周期数。         |
| pal\_mte\_mme\_cycle   | vme和mme指令并行运行周期数。         |
| pal\_vme\_mte\_cycle   | vme和mte指令并行运行周期数。         |
| pal\_total\_cycle      | vme、mme、mte总并行运行周期数。      |
| mte\_icmov\_cycle      | icmov指令周期数。               |
| mte\_l12llb\_cycle     | mov\_l12llb指令周期数。         |
| mte\_llb2l1\_cycle     | mov\_llb2l1指令周期数。         |
| mte\_pld\_cycle        | pld指令周期数。                 |
| mte\_pld\_byte         | pld搬移的数据总量，字节为单位。         |
| mte\_l12llb\_byte      | mov\_l12llb搬移的数据总量，字节为单位。 |
| mte\_llb2l1\_byte      | mov\_llb2l1搬移的数据总量，字节为单位。 |
| mte\_icmov\_byte       | icmov搬移的数据总量，字节为单位。       |
| syn\_wait\_cycle       | syn等待的周期数。                |
| vec\_slot\_wait\_cycle | rvv指令等待执行的周期数。            |
| mme\_slot\_wait\_cycle | mme指令等待执行的周期数。            |
| mte\_slot\_wait\_cycle | mte指令等待执行的周期数。            |
| l1\_conflict\_cycle    | l1冲突周期数。                  |
| im\_conflict\_cycle    | imb冲突周期数。                 |
| dcache\_miss\_cnt      | dcache没命中次数。              |
| icache\_miss\_cnt      | icache没命中次数。              |
| instruction\_cnt       | 总指令条数。                    |
...
2. 获取模型的NPU数据。
   ...
   ![](\doc_img\version-1.9.0\mltc-image-5.png)
...
4. 运行工具中的NPU数据拼接脚本，对NPU数据进行拼接。
   ...
   ![](\doc_img\version-1.9.0\mltc-image-6.png)
...
* dump\_data文件夹：主要保存的是dump出来的16进制的数据。经过`run_npu_data_analysis.py`脚本转成二进制形式并进行数据拼接，最终输出到npu文件夹。
...
查看cpu\_vs\_npu.csv表中的节点误差。

![](\doc_img\version-1.9.0\mltc-image-7.png)

主要查看nan/inf、errors/total和cos\_sim三列，分析精度问题：

* nan/inf列有数据显示True，说明出现过NaN或inf数据，可能存在精度问题。

* errors/total列不满足用户设置的相对误差和绝对误差的个数占比太大，可能存在现精度问题。

* cos\_sim列数值小于0.95，可能存在精度问题。
...
* 错误节点对应的绝对误差和相对误差散点图（`{op_name}_diff.png`）：
  ...
  ![](\doc_img\version-1.9.0\mltc-image-8.png)

* CPU和NPU数据的直方图分布（`{op_name}_hist.png`）：

  ![](\doc_img\version-1.9.0\mltc-image-9.png)

* 相对误差和绝对误差的直方图分布（`{op_name}_diff_hist.png`）：

  ![](\doc_img\version-1.9.0\mltc-image-10.png)
...
![](\doc_img\version-1.9.0\mltc-image-15.png)
...
| cos\_sim                                  | NPU结果和CPU结果的余弦相似度，数据越大越好。                                                              |
| 1st\_diff\_offset/offset\_dim/data\_shape | 第一个数据错误的位置，对应的维度信息，数据shape信息。                                                          |
| ref\_value                                | 标准模型（TensorFlow模型、ONNX模型）第一个发生错误时的数据（目前仅保留小数点后6位有效数字），若当前算子均未发生精度对比失败，此时的值为算子输出的第一个数据。 |
| npu\_value                                | 基于九章NPU框架的推理模型，第一个发生错误时的数据（目前仅保留小数点后6位有效数字），若当前算子均未发生精度对比失败，此时的值为算子输出的第一个数据。           |
| max\_abs\_err                             | 节点错误数据的最大绝对误差。                                                                         |
| mean\_abs\_err                            | 节点错误数据的平均绝对误差。                                                                         |
| max\_abs\_total                           | 节点数据的最大相对误差。                                                                           |
| mean\_abs\_total                          | 节点数据的平均相对误差。                                                                           |
...
![](\doc_img\version-1.9.0\mltc-image-16.png)
...
| -------------- | ---------------- |
| index          | 精度对比失败发生的索引号。    |
| std\_value     | 标准模型相应索引位置的值。    |
| npu\_value     | 九章NPU模型相应索引位置的值。 |
| abs\_diff      | 绝对误差。            |
| relative\_diff | 相对误差。            |
...
* 随机精度问题定位
  ...
  ![](\doc_img\version-1.9.0\mltc-image-17.png)

  比较两个dump路径下的`result/md5.csv`文件就能知道那个节点的输出存在随机问题。

  此外可以观察md5值是否是变化，来判断网络是否存在随机精度问题。

  ![](\doc_img\version-1.9.0\mltc-image-18.png)
...
* 重复/丢失节点问题
  ...
  * 重复节点问题：
  ...
  ![](\doc_img\version-1.9.0\mltc-image-19.png)

  * 丢失节点问题：

    节点名重复导致信息不一致，会出现节点丢失的现象，工具在分析过程中中会报以下错误：

    ![](\doc_img\version-1.9.0\mltc-image-20.png)

  上述两个节点问题的错误日志会记录在`result/skip_file_dict.csv`，如下所示：

  ![](\doc_img\version-1.9.0\mltc-image-21.png)
...
SNQ量化工具可以对小模型进行PTQ量化，量化工具能够直接支持的模型格式有ONNX、CAFFE，其他模型格式需先转成ONNX。若有PyTorch大模型量化需求，可选择使用SNC量化工具，详情可参见STC\_LLM使用指南。



![](\doc_img\version-1.9.0\mltc-image-22.png)

> 说明：如果在GPU上使用工具，请安装CUDA 11.8版本、CUDA驱动（cuda-repo-ubuntu2004-12-1-local\_12.1.0-530.30.02-1\_amd64.deb）以及显卡。
...
2. 启动量化命令，生成量化后模型和模型参数。
   ...
   1. 将量化后的模型和分离的模型参数一起放到NPU上使用MLTC编译运行。具体编译运行方式可参考在NPU上部署模型章节。

### 代码示例

Bert128模型预处理脚本（pre\_process.py），请自行准备好Bert128的数据集和Bert128的ONNX模型。
...
1. 获取可视化IR。
   ...
   ```Bash
   $ python slim.py --mlir_path /root/stcrp190/script/mlir_files/resnet50.mlir --out_path /root/stcrp190/script/resnet50_view.mlir
   ```
